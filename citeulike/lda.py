...
_GSL_LIB_PATH = "/home/2013/ptruon4/citeulike/lda/gsl/gsl_installed/lib"
_LDA_BINARY = "/home/2013/ptruon4/citeulike/lda/lda_topics/lda"
_VOCABULARY_PATH = "/home/2013/ptruon4/citeulike/lda/lda_topics/vocab.dat"
...
MAX_PROCESSING_LENGTH = sys.maxsize
...
def _word_counter_to_matrix_text(word_counter):
    ...
    output = ['{0}:{1}'.format(x[0], x[1]) for x in output]

    return '{0} {1}'.format(len(output), ' '.join(output)) if output else ''
...
def _grade(lda_result, word_counters):
    ...
    """
    row_list = [_word_counter_to_matrix_row(element) for element in word_counters]
    ...
    output = preprocessing.scale(output)
    return output.T
...
def extract_lda(papers, clean_up = True):
    ...
    data = [_word_count(word) for word in paper_abstracts]
    temp_dir = _to_data_file([_word_counter_to_matrix_text(row) for row in data])
