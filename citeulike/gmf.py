...
def to_mat(d, nusers, nitems):
  ...
  # return mat_row.todense(), mat_col.todense()
...
def optimize(m, train_data, valid_data, nusers, nitems, T, max_iters = np.inf):
  ...
  while True and iter < max_iters:
    ...
    logger.info("Starting to update UV.")
    current = time.time()
    m.update_u(train_data_row)
    m.update_v(train_data_col)
    logger.info("Update uv took {} seconds.".format(time.time() - current))
...
def solve_reg_least_squares_par(d, U, V, a, b, lambda_):
  ...
  U = Parallel(n_jobs=5)(delayed(solve_reg_least_squares_oneuser)(d[u,:], U[u,:], V, bVV, a_m_b, a) for u in range(U.shape[0]))
...
class Model:
  ...
  def mse_err(self, data):
    # preds = map(lambda i: self.predict_pair(data[i,0], data[i,1]), range(data.shape[0])) # For python 2
    preds = np.array([self.predict_pair(data[i,0], data[i,1]) for i in range(data.shape[0])]) # For python 3
