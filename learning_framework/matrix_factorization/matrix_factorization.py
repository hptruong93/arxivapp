...
class ModelData(object):
    ...
    def map_paper_data(self):
        """
            Generate paper id --> index and index --> paper id
        """
        self.index_paper_map = {}
        self.index_paper_reversed_map = {}
        ...
        count = 0
        for paper in doing:
            self.index_paper_map[paper.arxiv_id] = count
            self.index_paper_reversed_map[count] = paper.arxiv_id
            count += 1
    ...
    def map_user_data(self):
        ...
        count = 0
        for user in doing:
            self.index_user_map[user.id] = count
            count += 1
    ...
    def map_uv(self):
        ...
        def _add_point(arxiv_id, user_id, point = 1):
            if arxiv_id not in self.index_paper_map:
                logger.info("Something wrong?")
                return

            user_index = self.index_user_map[user_id]
            paper_index = self.index_paper_map[arxiv_id]

            if user_index not in self.train_points:
                self.train_points[user_index] = {}

            if paper_index in self.train_points[user_index]:
                self.train_points[user_index][paper_index] += point
            else:
                self.train_points[user_index][paper_index] = point
    ...
    def map_category(self):
        ...
        """
        self.category_map = {}
        self.category_reversed_map = {}

        logger.info("Collecting categories")
        index = 0
        for category in models.Category.objects.all():
            self.category_map[category.code] = index
            self.category_reversed_map[index] = category.code
            index += 1
    ...
    def generate_v_matrix(self):
        ...
        for paper in papers:
            paper_index =  self.index_paper_map[paper.arxiv_id]

            # Category part of the matrix
            for category in paper.categories.all():
                category_index = self.category_map[category.code]

                if paper_index not in self.v_matrix:
                    self.v_matrix[paper_index] = {}

                self.v_matrix[paper_index][category_index] = 1
            ...
            for topic_index, topic_value in enumerate(lda_result[paper_index]):
                ...
                self.v_matrix[paper_index][config.CATEGORY_COUNT + topic_index] = topic_value


    def print_output(self):
        logger.info("Printing to files")
        write_dict(self.index_paper_map, os.path.join(DATA_ROOT, 'paper_map.tsv'))
        write_dict(self.index_user_map, os.path.join(DATA_ROOT, 'user_map.tsv'))
        write_dict(self.category_map, os.path.join(DATA_ROOT, 'category_map.tsv'))
        write_double_dict(self.v_matrix, os.path.join(DATA_ROOT, 'V.tsv'))

        self.v_matrix = {} # Free memory

        while True:
            to_train_points, non_train_points = split_dict(self.train_points, 0.8)
    ...
    def extract_data(self):
        lda.load_vocabulary()
        django_db.close_connection() # To avoid having a connection being inactive for too long

        execution = [self.map_paper_data,
                    self.map_user_data,
                    self.map_category,
...
class MatrixFactorization(learning_interface.LearningInterface):
    ...
    def extract_data(self):
        self.data_object = ModelData()
        self.data_object.extract_data()
        return True, "Data extracted"

    def split_data(self):
        self.data_object.split_data()
        return True, "Split data"

    def _train(self, data_object, k, lambda_u, lambda_v, data_name = 'TEST', input_nneg = False, input_nitems = None, input_nusers = None, input_load_data = False):
        nusers, nitems, train, valid, test, idx0_user, idx0_item = gmf.load_data(DATA_ROOT)
        nitems = max(nitems, len(data_object.index_paper_reversed_map))
        nusers = max(nusers, len(data_object.index_user_map))
        ...
        output_model = gmf.Model(nusers, nitems, b=b, k=k, u_vprior=lambda_u, v_vprior=lambda_v, experiment_dir=eval_dir,nneg=input_nneg,idx0_user=idx0_user, idx0_item=idx0_item)

        if not input_load_data:
            errs = gmf.run(output_model, train, valid, test, nusers, nitems, 100)
        else:
            ...
            output_model.load(U,V)
            logger.info('+ U&V loaded')
            errs = gmf.run(output_model, train, valid, test, nusers, nitems, 100, test_only=False)
    ...
    def train(self, k, lambda_u, lambda_v, data_name = 'TEST', input_nneg = False, input_nitems = None, input_nusers = None, input_load_data = False):
        new_model = self._train(self.data_object, k, lambda_u, lambda_v, data_name, input_nneg, input_nitems, input_nusers, input_load_data)
        self.model = new_model
        return True, "Train data"

    def retrain(self, k, lambda_u, lambda_v, input_load_data):
        data_object = ModelData()
        data_object.extract_data()
        data_object.split_data()

        new_model = self._train(data_object, k, lambda_u, lambda_v, input_load_data)

        # Assume atomic operation since they are so short
    ...
    def predict(self, user_id):
        ...
        if user_id not in self.data_object.index_user_map:
            message = "Cannot find user with id {0}".format(user_id)
            return False, message

        user_index = self.data_object.index_user_map[user_id]
        if self.model is None:
            return False, "Model is None"

        result = self.model.predict_user(user_index, max_items = 10)
        result = [self.data_object.index_paper_reversed_map[paper_index] for paper_index in result if paper_index in self.data_object.index_paper_reversed_map]
    ...
    def sort(self, user_id, papers):
        ...
        paper_indices = [(index, self.data_object.index_paper_map[paper_id]) for index, paper_id in enumerate(papers) if paper_id in self.data_object.index_paper_map]
        # Leave uknown papers separated
        uknown_papers = [index for index, paper_id in enumerate(papers) if paper_id not in self.data_object.index_paper_map]
